...
export const videoCaptionSchema = z.object({
  /** Human-readable track name from upstream ("English", "français (auto-généré)"). */
  label: z.string(),
  /** BCP-47-ish code from upstream (`code` on Piped, `language_code` on Invidious). */
  languageCode: z.string(),
  /** Absolute URL to a WebVTT track. */
  url: z.string().url(),
...
export const videoDetailSchema = z.object({
  ...
  /** Subtitle tracks from upstream. Clients that can't use embedded HLS tracks fetch these. */
  captions: z.array(videoCaptionSchema).optional(),
  /** Parsed from the description server-side so every client gets them for free. */
