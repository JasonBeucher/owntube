...
/**
 * Subtitle tracks, normalized across upstreams. Piped `/streams` returns
 * `subtitles: [{ url, name, code, autoGenerated }]`; Invidious returns
 * `captions: [{ label, language_code, url }]` with a relative URL. Both are
 * WebVTT. Unparseable entries are dropped rather than failing the whole detail.
 */
export function mapUpstreamCaptions(
  value: unknown,
  baseUrl: string,
  ...
  for (const entry of value) {
    ...
    const url = resolveInvidiousAbsoluteMediaUrl(
    ...
    const languageCode =
      typeof o.code === "string" && o.code.trim()
        ? o.code.trim()
        : typeof o.language_code === "string" && o.language_code.trim()
          ? o.language_code.trim()
          : "";
    const label =
      typeof o.name === "string" && o.name.trim()
        ...
        : typeof o.label === "string" && o.label.trim()
          ? o.label.trim()
          : languageCode;
    if (!label || !languageCode) continue;
    seen.add(url);
    out.push({
      label,
      languageCode,
      url,
      autoGenerated: o.autoGenerated === true ? true : undefined,
    });
  }
